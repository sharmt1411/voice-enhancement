...
def audio_denoise(audio, method=None, reference_noise=None, mu=0.01, filter_order=10, noise_reduction_factor=0.95,
    ...
    # 保存重建的音频文件
    # sf.write(f'../test_data/test_denoise/denoise_audio_{method}.wav', audio, sr)
    save_audio(audio, sr, f'../test_data/test_denoise/denoise_audio_{method}.wav')
    return audio


def perform_stft(audio, n_fft, hop_length):
    """短时傅里叶变换(STFT)"""
...
def preprocess_audio(file, n_fft, hop_length, is_file_path=True):
    """音频预处理主函数"""
    # 加载音频
    if is_file_path:
    ...
    # 短时傅里叶变换
    magnitude, phase = perform_stft(audio_normalized, n_fft=n_fft, hop_length=hop_length)
    print("stft_end_time", datetime.now())
    # 返回预处理后的结果
    return magnitude, phase, sr


def mel_to_audio(mel_spectrogram, n_fft, hop_length, sr=16000, n_iter=32):
    """
    ...
    """
    # 将Mel谱图转换为stft
    mel_inverted_spectrogram = librosa.feature.inverse.mel_to_stft(mel_spectrogram, 
                                                                   sr=sr, n_fft=n_fft)

    # 使用Griffin-Lim算法估计相位并重建音频
    audio_reconstructed = librosa.griffinlim(mel_inverted_spectrogram, 
                                             n_fft=n_fft, n_iter=n_iter, hop_length=hop_length)

    return audio_reconstructed


def audio_to_logmel_mel(audio, n_fft, hop_length, n_mels, sr=16000, fmax=8000):
    """
    将音频信号转换为Mel频谱图
...
def audio_to_mel(audio, n_fft, hop_length, sr=16000, n_mels=128, fmax=8000, ):
    # 计算Mel频谱
    mel_spectrogram = librosa.feature.melspectrogram(y=audio, sr=sr, n_mels=n_mels, fmax=fmax,
                                                     n_fft=n_fft, hop_length=hop_length)
...
def mel_align(mel1, mel2):
   ...
   :return: 对齐后的两个Mel频谱图
    ...
    if align_shape:
        ...
        corr = correlate(mel1_mask.sum(axis=0), mel2_mask.sum(axis=0))  
        # 长度为len(A) + len(B)-1,将b在a上滑动，并逐步计算两者的点积，结果是每个偏移位置的相关性值。
    else:
        # 计算两个 Mel 图之间的交叉相关
        # 由于mel图频率分布差异，所以不能平均后测试相关，待测试？而是直接求和后测试相关
        corr = correlate(mel1[64:128].sum(axis=0), mel2[64:128].sum(axis=0))  
        # 长度为len(A) + len(B)-1,将b在a上滑动，并逐步计算两者的点积，结果是每个偏移位置的相关性值。
    # print("corr", corr)
    # 找到最大相关值的位置（对应的时间偏移量）
...
def plot_mel_spectrogram_list(mel_list, sr=16000, n_mels=128, fmax=8000, is_log=True):
    ...
    """
    if not is_log:
        log_mel_list = [(librosa.amplitude_to_db(mel_spectrogram, ref=np.max), method) 
                        for mel_spectrogram, method in mel_list]
    else:
...
def plot_mel_hist(mel_list):
    ...
    for i, (mel_spectrogram, message) in enumerate(mel_list):
        ...
        plt.scatter(np.arange(len(mel_spectrogram.flatten())), mel_spectrogram.flatten(), alpha=0.5, label=message)
        # plt.hist(mel_spectrogram.flatten(), bins=200, density=False, alpha=0.5, label=message)
        # plt.xlim(-10,10)  # 设置X轴的最小值和最大值
        # plt.grid()  # 显示网格
        # plt.xlabel('value')
...
if __name__ == "__main__":
    ...
    def test_audio_denoise():
        # 请替换为你自己的音频文件路径
        audio_path = "../test_data/example.wav"
        ...
        n_fft = 2048
        hop_length = 512
        log_mel_original, mel_original = audio_to_logmel_mel(y, n_fft=n_fft, hop_length=hop_length, sr=sr, n_mels=n_mel, fmax=8000)
        mel_list = [(log_mel_original, "_original")]
        audio_reconstructed = mel_to_audio(mel_original, n_fft=n_fft, hop_length=hop_length, sr=sr)
        # sf.write(f'../test_data/test_noise/reconstructed_audio_original.wav', audio_reconstructed, sr)
        save_audio(audio_reconstructed, sr, f'../test_data/test_denoise/reconstructed_audio_original.wav')
        for r in range(1, len(method) + 1):
            combinations = list(itertools.combinations(method, r))
            for combo in combinations:
                print("测试方法：", combo, "开始--------------------------------------------------")
                y, sr = librosa.load(audio_path)
                audio_denoise_result = audio_denoise(y, method=combo)  # 只取保存好的降噪音频
                log_mel_spectrogram, mel_spectrogram = audio_to_logmel_mel(
                    audio_denoise_result, n_fft=n_fft, hop_length=hop_length, sr=sr, n_mels=n_mel, fmax=8000)
                mel_list.append((log_mel_spectrogram, str(combo)+"_denoise"))
                print("mel_spectrogram_shape", log_mel_spectrogram.shape)
                audio_reconstructed = mel_to_audio(mel_spectrogram, n_fft=n_fft, hop_length=hop_length, sr=sr)
                # sf.write(f'../test_data/test_noise/reconstructed_audio{str(combo)}.wav', audio_reconstructed, sr)
                save_audio(audio_reconstructed, sr, f'../test_data/test_denoise/reconstructed_audio{str(combo)}.wav')
                log_mel_spectrogram_reconstructed, _ = audio_to_logmel_mel(
                    audio_reconstructed, n_fft=n_fft, hop_length=hop_length, sr=sr, n_mels=n_mel, fmax=8000)
                mel_list.append((log_mel_spectrogram_reconstructed, str(combo)+"_reconstructed"))

        # plot_mel_spectrogram_list(mel_list, sr=sr, n_mels=n_mel, fmax=8000, is_log=True)
        print("测试结束-")
